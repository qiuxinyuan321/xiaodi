...
如果目标目录不是 git 仓库，记得补上 `--skip-git-repo-check`，这样 Codex 才会跳过仓库检测，直接在普通目录里执行。
...
独立的子任务不需要排队，小弟可以同时派出多个分身：

```
Claude Code 拆分计划
    ├── Codex 实例 A → 改 auth 模块
    ├── Codex 实例 B → 改 logging 模块
    └── Codex 实例 C → 改 test fixtures
         ↓ 全部完成
Claude Code 统一审查 + 冲突检测
```

适用于：多模块重构、批量文件修改、独立测试套件。

## 成本追踪

每次执行后，小弟会报告 token 消耗明细：
